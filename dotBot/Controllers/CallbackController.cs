...
public class CallbackController : ControllerBase
{
    ...
    public void CheckMessage(ApplicationContext db, Message message)
    {
        ...
        {
            sqlCommand = "INSERT INTO peer" + message.peer_id + " VALUES ( " + message.from_id + ", '" + db.Users.Find(message.from_id).Name + "', 0, 0, 0, 0)";
            sendSqlCommand(sqlCommand);
        }
    ...
    private void marriageRequest(Message message)
    {
        ...
        if (_requester.Marry == 0)
        {
            if (_target.Marry == 0)
            {
                result = "[id" + _requester.Id.ToString() + "|" + _requester.Name + "]" + " предложил(а) " + "[id" + _target.Id.ToString() + "|" + _target.Name + "]" + " брак" + "\n" + "Для ответа: Брак да / нет";
                _target.MarryageRequest = _requester.Id;
            }
            else
            {
                result = "[id" + _target.Id.ToString() + "|" + _target.Name + "]" + " состоит в браке c " + "[id" + _target.Id.ToString() + "|" + _target.Name + "]";

            }
        }
        else
        {
            result = "[id" + _requester.Id.ToString() + "|" + _requester.Name + "]" + " состоит в браке c " + "[id" + _target.Id.ToString() + "|" + _target.Name + "]";
        }
        db.SaveChanges();
        VKAnswer(message.peer_id, result);
    }
    private void marriage(Message message)
    {
        ...
        switch (message.text.ToLower())
        {
            case "брак да":
                ...
                result = "[id" + _user1.Id.ToString() + "|" + _user1.Name + "]" + " и " + "[id" + _user2.Id.ToString() + "|" + _user2.Name + "]" + " теперь состоят в браке";
                _user1.MarryageRequest = 0;
                break;
            case "брак нет":
                result = "[id" + _user2.Id.ToString() + "|" + _user2.Name + "]" + " не хочет брак(((((";
                _user1.MarryageRequest = 0;
                break;
    ...
    private void divorce(Message message)
    {
        ...
        if (_gs1.Marry != 0)
        {
            _gs1.Marry = 0;
            _gs2.Marry = 0;
            result = "[id" + _gs1.Id.ToString() + "|" + _gs1.Name + "]" + " и " + "[id" + _gs2.Id.ToString() + "|" + _gs2.Name + "]" + " больше не в браке. Поздравляю!!!";
        }
        else
        {
    ...
    private async void kick(Message message)
    {
        ...
        if (_gs2.hp > 0)
        {
            ...
            if (_gs2.defence < _gs.power)
            {
                ...
                result += "[id" + message.from_id + "|" + db.Users.Find(message.from_id).Name + "]" + " ударил(а) " + "[id" + message.reply_message.from_id + "|" + db.Users.Find(message.reply_message.from_id).Name + "]" + "\n";
    ...
    //вывод браков (дублируются поля при выводе юз1 - юз2, юз2 - юз1)
    private async void marryes(Message message)
    {
        string sqlCommand = "SELECT Id, Marry FROM Users";
        var db = getNewDbContext();
        string result = "Браки беседы\n";
        SqlDataReader reader = sendSqlCommand(sqlCommand);
        while (reader.Read())
        {
    ...
    private void admKick(Message message)
    {
        ...
        string result;
        string requester = db.Users.Find(message.from_id).Name;
        string target = db.Users.Find(message.reply_message.from_id).Name;
        if (senderRole >= 1)
        {
            sendSqlCommand(sqlCommand);
